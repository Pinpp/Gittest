...
def first_and_last_image(data):
    ...
    for n_data in range(len(data[0])):
        i = [data[0][n_data],data[1][n_data],data[2][n_data],data[3][n_data],data[4][n_data],data[5][n_data],data[6][n_data],data[7][n_data]]
        if i[1:4] not in mark_list:
            mark_list.append(i[1:4])
            status_dict = dict(B_UT = i[0],E_UT = i[0],Image_RA = i[4],Image_DEC = i[5],CCD_ID = i[6],CCD_TYPE = i[7])
