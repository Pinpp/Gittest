...
#    print sys.argv  
    if not sys.argv[1:]:
        sys.argv += [ "2019-07-01T12:04:36", "2019-07-01T18:04:36", "./"]
        ...
        for nin in indices:
            ...
            if len(data[0]) >= 1:         
                ...
                for k in all_list:
                    ...
                    outline = '%s %05.2f %05.2f %s %s %s %s %s %s %s %s %s %s %s %s %s' % (Grid_ID[nin],Field_ID[nin],float(Pointing_RA[nin]),float(Pointing_DEC[nin]),B_UT , E_UT, M_coor_ra_deg, M_coor_dec_deg, M_coor_ra, M_coor_dec, Image_coor_ra_deg, Image_coor_dec_deg, Image_coor_ra, Image_coor_dec, CCD_ID, CCD_TYPE)
                    print(outline)
                    p += 1
