...
def check_obj_status(obj,obj_infs,pd_log_id):
    ...
    if group_id == 'XL001':
        ...
        print "\nCurrent obj : %s %s %s %s %s %s %s %s %s." % (type_tel[group_id], obj, unit_id, obj_name, objra, objdec, str(int(float(expdur))), frmcnt, priority)
        #print "\nChecking it in log ..."
    if group_id in ['XL002','XL003']:
        ### check the gft,log_dist and cam_log
        print "\nCurrent obj : %s %s %s %s %s %s %s %s %s %s." % (type_tel[group_id], obj, unit_id, obj_name, objra, objdec, filter, str(int(float(expdur))), frmcnt, priority)
        #print "\nChecking it in log ..."
    if log_dist_id:
        stage = 'Observing'
    else:
...
def check_sent_new():
    while True:
        ...
        t1.setDaemon(True)
        t1.start()
        t2 = threading.Thread(target=check_sent_main,args=('XL001',))
        t2.setDaemon(True)
        t2.start()
        t3 = threading.Thread(target=check_sent_main,args=('XL001',))
        t3.setDaemon(True)
        t3.start()
        time.sleep(1)
        t1.join()
...
def check_new():
    while True:
        ...
        if msg:
            ...
            #sync()
            mark_wait = 0
            t_cn1 = Mythread_get_result(target=check_new_main,args=('XL001',))
            t_cn1.setDaemon(True)
            t_cn1.start()
            t_cn2 = Mythread_get_result(target=check_new_main,args=('XL002',))
            t_cn2.setDaemon(True)
            t_cn2.start()
            t_cn3 = Mythread_get_result(target=check_new_main,args=('XL003',))
