    ...
    while True:
        ...
        if not mark_id:
            ...
            if ids:
                ...
                for id in ids:
                    fo_id = id[0]
                    sql = "SELECT fo_name, ra, dec, epoch, expose_duration, frame_count, filter, priority, trigger_time, obj_name FROM follow_up_observation WHERE fo_id = " + str(fo_id)
                    res = sql_act('gwac_db',sql)
                    if res:
                        run_name, objra, objdec, epoch, expdur, frmcnt, filter, priority, trigger_time, obj_name = res[0][:]
                        try:
                        ...
                        obj_line = "obj_name=%s objsour=GWAC_followup observer=GWAC objra=%s objdec=%s objepoch=%s objerror=0.0|0.0 objrank=0 group_id=XL002 unit_id=001 obs_type=goa obs_stra=pointing date_beg=%s date_end=%s day_int=0 imgtype=object filter=%s expdur=%s delay=0 frmcnt=%s priority=%s run_name=%s note=%s mode=observation\n" % \
                            (obj_name, objra, objdec, epoch, date_indb, date_indb, filter, expdur, frmcnt, priority, run_name, str(fo_id))
        ...
        if not mark_id:
            ...
            if ids:
                ...
                for id in ids:
                    ...
                    #print type(res[0][0])
                    run_name, objra, objdec, epoch, expdur, frmcnt, filter, priority, trigger_time, obj_name = res[0][:]
                    print run_name, objra, objdec, epoch, expdur, frmcnt, filter, priority, str(trigger_time)[:-3], obj_name
                    try:
                        date_indb = datetime.datetime.utcfromtimestamp(time.mktime(time.strptime(str(trigger_time)[:-3], "%Y-%m-%d %H:%M:%S.%f"))).strftime("%Y/%m/%d")
                    except:
                        date_indb = datetime.datetime.utcnow().strftime("%Y/%m/%d")
                    #print date_indb
                    obj_line = "obj_name=%s objsour=GWAC_followup observer=GWAC objra=%s objdec=%s objepoch=%s objerror=0.0|0.0 objrank=0 group_id=XL002 unit_id=001 obs_type=goa obs_stra=pointing date_beg=%s date_end=%s day_int=0 imgtype=object filter=%s expdur=%s delay=0 frmcnt=%s priority=%s run_name=%s note=%s mode=observation\n" % \
                        (obj_name, objra, objdec, epoch, date_indb, date_indb, filter, expdur, frmcnt, priority, run_name, str(id[0]))
                    print obj_line
                    lf_fu.write(obj_line + str(trigger_time) + '\n')
                    time.sleep(1)
                #print fo_id
