                ...
                for log in logs:
                    ...
                    if res:
                        for item in res:
                            ...
                            # if item_com_time:
                            #     item_com_time = item_com_time.group(0)
                            # else:
                            #     break
...
def check_obj_status(obj,obj_infs,pd_log_id):
    ...
    if group_id in ['XL002','XL003']:
        ...
        print "\nObj: %s %s %s %s %s %s %s %s %s %s." % (type_tel[group_id], unit_id, obj, obj_name, objra, objdec, filter, str(int(float(expdur))), frmcnt, priority)
        #print "\nChecking it in log ..."
    ...
    if check_log_dist_res:
        ...
        if res:
            ...
            if obs_stag == 'sent':
                ...
                if  sent_delay > 600: ### wait for 10 mins
                    #if group_id == 'XL003':
                ...
                if res:
                    ...
                    if obs_stag == 'sent':
                        ...
                        if group_id in ['XL002','XL003']:
                            ...
                            if dist_delay > 600:### wait for 30 mins
                                # if group_id == 'XL003':
                                #     cam_ip, cam_port = get_cam_config('3')[:]
                                #     cmd = 'autostop && autostart'
                                #     pd_socket_client(cam_ip, cam_port, cmd)
                            ...
                            if int(obj_pri) > int(pri_list[-1]):
                                if group_id in ['XL002','XL003']:
                                    if not check_tw_end(obj_sent_list):
                                        ...
                                        print '\n##### %s %s: Waitting for clear.' % (type_tel[group_id], unit_id)
                                else:
                                    print '\n##### %s %s: Higher Priority, %s, %s' % (type_tel[group_id], unit_id, obj, obj_pri)
                                    go_sent = 1
                            else:
                                if group_id == 'XL002' and len(pri_list) < 2:
